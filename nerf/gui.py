...
refinepose=[]
class NeRFGUI:
    def __init__(self, opt, trainer, train_loader=None, device= None, debug=True):
    ...
    def train_step(self,i):
        ...
        # visualize_poses(self.trainer.train_dataset.poses.cpu().numpy())
        starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
        starter.record()
        # if i>0:
        #     import main_nerf as mn
        #     from nerf.provider import NeRFDataset
        #     a = self.trainer.fget_graph()
        #     # self.train_loader = NeRFDataset(opt=self.opt, device=self.device, type='train').update_pose(newposes,self.trainer.fget_BARF(), self.trainer.fget_graph())
        #     # self.
        #     #self.train_loader = NeRFDataset(opt=self.opt, device=self.device, type='train')
        #     # self.train_loader.poses = self.trainer.update_pose(newposes)
        #     # size = len(self.train_loader.poses)
        #     # from torch.utils.data import DataLoader
        #     # self.train_loader = self.trainer.update_rays(self.train_loader,self.train_loader.poses, self.cam.intrinsics, self.W, self.H)
        #     # self.train_loader = DataLoader(list(range(size)), batch_size=1, collate_fn=self.collate, shuffle=self.training, num_workers=0)

        #데이터 로드 새로해서 warp한 이미지 업데이트
        from nerf.provider import NeRFDataset
        # from .nerf/provider import NeRFDataset
        self.train_loader = self.trainer.train_dataset.dataloader()
        outputs = self.trainer.train_gui(self.train_loader, step=self.train_steps)

        # from nerf.provider import NeRFDataset
        # self.train_loader = NeRFDataset(opt=self.opt, device=self.device, type='train').update_pose(newposes)

        ender.record()
        torch.cuda.synchronize() #현재 작업이 끝나기 전에 비동기 과정이 생기지 않게 한다.
    ...
    def register_dpg(self):
        ...
        dpg.create_viewport(title='3D Urban Map Builder', width=self.W, height=self.H, resizable=False)
    ...
    def render(self):
        ...
        while dpg.is_dearpygui_running():
            ...
            if self.training: #start 누르면 train
                self.train_step(i)
                i +=1
            self.test_step()
