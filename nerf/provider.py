...
class NeRFDataset:
    def __init__(self, opt, device,type='train', downscale=1, n_test=10):
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
                ...
                pose = np.array(f['transform_matrix'], dtype=np.float32) # [4, 4] #train_json
        ...
        np.save('original_pose', self.poses)
    ...
    def collate(self, index):
        ...
        if self.rand_pose == 0 or index[0] >= len(self.poses):
            ...
            rays = get_rays(poses, self.intrinsics / s, rH, rW, -1,None,self.i)
...
#self.poses = [B,4,4]
        poses = self.poses[index].to(self.device) # [1, 4, 4]
        # self.index.append(index)
        # update_pose22(self, newposes, opt, graph)
        error_map = None if self.error_map is None else self.error_map[index]
        #poses는 change nerf한 1,4,4
    ...
    def dataloader(self):
        #barf
        #trainer = Trainer('ngp')
        #self.poses =trainer.update_pose(newposes)

        size = len(self.poses)
        if self.training and self.rand_pose > 0: #X
            size += size // self.rand_pose # index >= size means we use random pose.
        # loader = DataLoader(list(range(size)), batch_size=1, shuffle=self.training, num_workers=0)
    ...
    # def update_pose(self, newposes,opt,graph):
    #     import main_nerf as mn
    #     # pose, pose_GT = mn.get_all_training_poses(opt, graph)
    #     pose = self.poses
    #     # newposes = []
    ...
    #         spose = nerf_matrix_to_ngp(spose, scale=0.33, offset=[0, 0, 0])
    ...
    #     self.radius = self.poses[:, :3, 3].norm(dim=-1).mean(0).item()
    #     #newposes=[]
    #     refinepose = self.poses
    #     size = len(self.poses)
    #     loader = DataLoader(list(range(size)), batch_size=1, collate_fn=self.collate, shuffle=self.training, num_workers=0)
    #     loader._data = self
    #     return loader

    def update_pose22(self, newposes):
        self.poses = newposes
