...
class NeRFRenderer(nn.Module):
    ...
    def run(self,index,rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
    ...
    def run_cuda(self,rays_o, rays_d, trainer,dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
       #  if(self.training):
       #      import random
       ...
       #      #1,4,4
       #      # trainer.model.
       #      poses = trainer.model.pose_train(trainer)
       #      index = random.sample(range(0, 40), 1)
       #      error_map = None if trainer.error_map is None else trainer.error_map[index]
       #      rays = get_rays(poses, trainer.train_dataset.intrinsics, trainer.train_dataset.H, trainer.train_dataset.W,
       ...
       # # ----------------
        ...
        if self.training:
            ...
            #plot_pointcloud(xyzs.reshape(-1, 3).detach().cpu().numpy())
            # else:
            xyzs, dirs, deltas, rays,nears,fars = self.pose_train(self.trainer,self)
            #-------------------------------------
            sigmas, rgbs = self(xyzs, dirs) #network forward
            ...
            while step < max_steps:
                ...
                # xyzs, dirs, deltas, nears, fars = self.pose_train(self.trainer, self)
                sigmas, rgbs = self(xyzs, dirs) #network forward
                # density_outputs = self.density(xyzs) # [M,], use a dict since it may include extra things, like geo_feat for rgb.
                # sigmas = density_outputs['sigma']
                # rgbs = self.color(xyzs, dirs, **density_outputs)
            ...
            a = np.array(weights_sum.cpu())
            b = np.array(depth.cpu())
            c = np.array(image.cpu())
        ...
        # dot = make_dot(rgbs, params={"x": self.model.parameters()})
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        tmp_grid = - torch.ones_like(self.density_grid) #입력 텐서와 동일한 모양의 텐서를 만들어주는데 1로 채워줌
    ...
    def render(self,rays_o, rays_d, trainer,staged=False, max_ray_batch=4096, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]
        trainer.dt_gamma = kwargs['dt_gamma']
        # trainer.mean_counter = self.mean_count
        if self.cuda_ray: #O
            _run = self.run_cuda
        else:
            _run = self.run
        ...
        if staged and not self.cuda_ray: #X
            ...
            results = _run(rays_o, rays_d,trainer, **kwargs) #wights_sum,depth,image
