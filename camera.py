...
class Lie():
    ...
    def se3_to_SE3_backward(self,wu, grad_output):
        R = grad_output[..., :3, :3].cuda()
        V = grad_output[..., :3, 3:].cuda()
        w, u = wu.split([3, 3], dim=-1)

        # R에 대한 그래디언트 계산
        wx = self.skew_symmetric(w)
        theta = w.norm(dim=-1)[..., None, None]
        A = self.taylor_A(theta).cuda()
        B = self.taylor_B(theta).cuda()
        C = self.taylor_C(theta).cuda()
        I = torch.eye(3, device=w.device, dtype=torch.float32)

        # R에 대한 그래디언트 계산
        grad_R = R @ (I + A * wx + B * wx @ wx)

        # V에 대한 그래디언트 계산
        # grad_V = V @ (I + B * wx + C * (wx @ wx))

        # u에 대한 그래디언트 계산
        grad_u = (V @ u.unsqueeze(-1)).squeeze(-1)

        # wu에 대한 그래디언트 계산
        grad_wu = torch.cat([grad_R, grad_u], dim=-1)
