...
class _march_rays_train(Function):
    ...
    def forward(ctx, rays_o, rays_d, bound, density_bitfield, C, H, nears, fars, step_counter=None, mean_count=-1, perturb=False, align=-1, force_all_rays=False, dt_gamma=0, max_steps=1024):
        ...
        _backend.march_rays_train(rays_o, rays_d, density_bitfield, bound, dt_gamma, max_steps, N, C, H, M, nears, fars, xyzs, dirs, deltas, rays, step_counter, noises) # m is the actually used points number
                                    #4096 4096    524288,           2      0.0078125  1024      4096 2 128 4194304 4096, 4194304 4194304 4194304 4096 2,          4096
...
class _composite_rays_train(Function):
    ...
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights_sum, grad_depth, grad_image): #사용
        # print("composite_rays backward")
        # print("grad_weights_sum: ", grad_weights_sum,"grad_depth: ",grad_depth,"grad_image: ",grad_image)
        ...
        # print("ray ctx: ",ctx.next_functions)
        ...
        torch.save(grad_rgbs, 'gradient_rgb.pth')
        # print("grad_rgbs", grad_rgbs.size())
