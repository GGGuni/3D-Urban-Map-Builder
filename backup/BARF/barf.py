...
class _pose_train(Function):
    ...
    def forward(ctx,se3,poses,device,trainer,network,index=None):#,  # poses : new pose , w : se3_refine
        ...
        if network.training :
            ...
            # ctx.save_for_backward(torch.from_numpy(M),torch.tensor(index[0]),se3)
    ...
    def backward(ctx, xyzs, dirs, deltas, rays,poses,nears,fars,se3):
        ...
        cur_ind = rays[0][0]
        tx = (xyzs[cur_ind][0] - rays_o[cur_ind][0]) / rays_d[cur_ind][0]
        ty = (xyzs[cur_ind][1] - rays_o[cur_ind][1]) / rays_d[cur_ind][1]
        tz = (xyzs[cur_ind][2] - rays_o[cur_ind][2]) / rays_d[cur_ind][2]

        # if tx==ty and ty==tz :
            # # 편미분 계산
        dr1 = directions[0][cur_ind][0] * tx
        dr2 = directions[0][cur_ind][0] * ty
        dr3 = directions[0][cur_ind][0] * tz
        dr4 = directions[0][cur_ind][1] * tx
        dr5 = directions[0][cur_ind][1] * ty
        dr6 = directions[0][cur_ind][1] * tz
        dr7 = directions[0][cur_ind][2] * tx
        dr8 = directions[0][cur_ind][2] * ty
        dr9 = directions[0][cur_ind][2] * tz
        ...
        if torch.any(torch.isnan(se3)) : pass
            ...
            grad_se3[index] = torch.mean(se3, dim=0, keepdim=True)
