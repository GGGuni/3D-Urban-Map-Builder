...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1,step=None):
    ''' get rays
    Args:
...
class Trainer(object):
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(len(train_loader)):
            ...
            # loss.backward()
            # self.loss.all.backward()
            self.scaler.scale(loss).backward(retain_graph=True) # Scales the loss, and calls backward() to create scaled gradients
            # self.train_dataset.pred_rgb[index[0]] = preds
            # loss.backward()
            # from BARF.barf import goutput
            # goutput = torch.load('gradient_rgb.pth')
            # grad_inputs = torch.load('grad_inputs.pth')
            # grad_dir = torch.load('grad_dir.pth')
            # print("gradient_rgb: ",goutput) #composite
            # print("grad_inputs: ", grad_inputs)
            # print("grad_dir: ", grad_dir)
            # gradient = torch.autograd.grad(loss, self.self_graph.se3_refine.parameters(),allow_unused=True)
